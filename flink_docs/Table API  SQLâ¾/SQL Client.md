...
The SQL Client scripts are also located in the binary directory of Flink. Users have two options for starting the SQL Client CLI, either by starting an embedded standalone process or by connecting to a remote SQL Gateway.
...
You can refer to the configuration section for
...
Attention The SQL Client does not track the status of the
...
use the job statements to monitor the detached query status
or stop the detached query.
...
The savepoint path could be specified with execution.checkpointing.savepoint-dir
